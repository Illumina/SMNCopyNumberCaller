...
def get_normed_depth(bamf, region_dic, nCores=1, reference=None,
                     gc_correct=True):
    """
    Return the normalized depth values and coverage stats for a sample
    given a bam file
    """
    counts_for_normalization, gc_for_normalization, region_type_cn, \
        read_length = count_reads_and_prepare_for_normalization(
            bamf, region_dic, nCores, reference)
    normed_depth = normalize(
        counts_for_normalization, gc_for_normalization, region_type_cn,
        read_length, gc_correct)

    return normed_depth


def get_normed_depth_from_count(count_file, region_dic, read_length,
                                gc_correct=True):
    """
    ...
    counts_for_normalization, gc_for_normalization, region_type_cn = \
        process_counts_and_prepare_for_normalization(count_dic, region_dic)
    normed_depth = normalize(
        counts_for_normalization, gc_for_normalization, region_type_cn,
        read_length, gc_correct)
...
def get_read_count(bamfile, region, mapq_cutoff=0):
    ...
    nreads = 0
    for read in reads:
        if (read.mapq >= mapq_cutoff and
                read.is_secondary == 0 and read.is_supplementary == 0 and
                read.reference_start >= region[1] and
                read.reference_start < region[2]):
            nreads += 1
    return nreads
...
def normalize(counts_for_normalization, gc_for_normalization, region_type_cn,
              read_length, gc_correct):
    """
    Return the normalized depth values for a sample.
    Median normalization and/or GC normalization
    """
    gc_corrected_depth = gc_correction(
...
def count_reads_and_prepare_for_normalization(bamf, region_dic, nCores=1,
                                              reference=None):
    """
    Return the normalized depth values and coverage stats for a sample
    given a bam file
    ...
    for region_type in region_dic:
        if region_type != 'norm':
            ...
            if region_length is None or hap_cn is None:
                raise Exception(
                    'Problem with region definition. Length not specified.')
    ...
    lregion = [(region[0], region[1], region[2], gc)

...
from ..snp_count import get_snp_position, get_supporting_reads, \
    get_supporting_reads_single_region, get_fraction
...
class TestReadCount(object):
    def test_get_snp_count(self):
        ...
        lsnp1, lsnp2 = get_supporting_reads(
            bam1, dsnp1, dsnp2, nchr, dindex)
        assert lsnp1 == [
            0, 0, 0, 0, 0, 0, 29, 35, 26, 39, 29, 35, 32, 37, 39, 39]
        assert lsnp2 == [
            0, 0, 0, 0, 0, 0, 12, 39, 39, 32, 26, 55, 45, 33, 42, 18]

        bam2 = os.path.join(test_data_dir, 'NA12885.bam')
        lsnp1, lsnp2 = get_supporting_reads(
            bam2, dsnp1, dsnp2, nchr, dindex)
        assert lsnp1 == [
            46, 32, 45, 36, 34, 14, 36, 54, 38, 34, 41, 41, 40, 51, 40, 37]
        assert lsnp2 == [
            35, 35, 32, 29, 35, 59, 22, 28, 32, 24, 34, 32, 33, 28, 38, 21]

        lsnp1, lsnp2 = get_supporting_reads_single_region(
            bam2, dsnp1, nchr, dindex)
        assert lsnp1 == [
            46, 32, 45, 36, 26, 14, 36, 54, 38, 34, 41, 41, 40, 51, 40, 34]
        assert lsnp2 == [
        ...
        lsnp1, lsnp2 = get_supporting_reads_single_region(
            bam2, dsnp1, nchr, dindex)
        assert lsnp1 == [
            46, 32, 45, 36, 26, 14, 36, 54, 38, 34, 41, 41, 40, 51, 40, 20]
        assert lsnp2 == [
            0, 0, 0, 0, 0, 11, 0, 0, 0, 0, 0, 0, 0, 0, 0, 16]
